...
enum StateRendererType {
  ...
  FULL_SCRENN_ERROR_STATE,

  CONTENT_SCREEN_STATE, //THE UI OF THE SCREEN

  EMPTY_SCREEN_STATE //EMPTY VIEW WHEN RECEIVE NOT DATA FROM API FOR LIST
}
...
class StateRenderer extends StatelessWidget {
  ...
  Widget _getStateWidget(BuildContext context) {
    switch (stateRendererType) {
      ...
      case StateRendererType.FULL_SCRENN_ERROR_STATE:
        return _getItemsInColumn([
          _getAnimatedImage(JsonAssets.error),
          _getMessage(message),
    ...
    return Center(
      child: Padding(
        ...
        child: Text(
          message,
          style:
              getMediumStyle(color: ColorManagar.black, fontSize: FontSize.s16),
        ),
      ),
    );
  }
          ...
          child: ElevatedButton(
              onPressed: () {
                if (stateRendererType ==
                    StateRendererType.FULL_SCRENN_ERROR_STATE) {
                  retryActionFunction
                      ?.call(); //to call the API function again to retry
                } else {
                  Navigator.of(context).pop(); //dismiss pop up error
                }
              },
